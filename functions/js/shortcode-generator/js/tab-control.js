function colabsTabMaker(h, i, f) {
    ...
    this.buildSelectControl = function () {
        ...
        this.selectControl = jQuery( "<select></select>").attr( "id", "colabs-tab-select").addClass(f ? f : "" );
    ...
    this.buildTextInputControl = function ( id ) {
    	...
    	var labelElement = '<label for="colabs_tab_title">Tab ' + id + ' Title</label>';
    	var inputElement = '<input type="text" id="colabs_tab_title_' + id.toString() + '" class="txt input-text" name="colabs_tab_title" />';
    ...
    this.buildTabButtons = function (a) {
        if (this.buttonsControl) {
			...
			jQuery( 'label[for="colabs_tab_title"]').parents( 'tr').remove();
